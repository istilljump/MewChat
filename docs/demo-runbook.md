...
**该看到**：`finalState=REPLY`，轨迹里仍有 `CONTEXT_LOAD`（说明读到了历史），
引用来自"七天无理由退换货规则"这篇 —— 即"这个"被消解成了上一轮的话题。
真模型下这一屏最能体现多轮记忆：它会用上"这个"指代的上下文，而不是把"这个"当成新问题。

> 用本地假端点时，这一屏的"消解"是**近似的**：它只是把上一句的话题并进来
> （否则"这个"会被当成检索词，指代对象就丢了），真模型会把它改写成
> "退换货有时间限制吗"这样的完整问句。演示时如果评审追问，如实说明这一点即可 ——
> 换了真 Key 就是真消解。

**指代消解是这一轮的关键一步，也是最值得讲的一处风险**：它会把 `resolvedMessage`
覆盖成模型输出的那句话，而下游的意图识别与检索用的就是它 ——
**模型在这一步返回垃圾，等于把用户的问题整句换掉**。
排查时如果发现"每类问题都被判成同一个意图"，先看日志里指代消解的输出
（`tools/fake-openai/fake-openai.log` 里有完整提示词与返回）。
...
若用真模型，回答会自然得多；但**数字来自工具**这一点两种路线都一样。
...
**该看到**：`done.citations` 非空，含 `docTitle`、`chunkNo`、`score`；
轨迹含 `RAG_RETRIEVE`。落库后再看 `message.ref_docs` 能看到同样的引用。

### 步骤 5：答不出来时兜底，并自动建单

```bash
ask "$S" "发票怎么开"          # 知识库里没有这个主题
```
