...
**已知局限（必须知悉）**：强推只是让旧提交在远端**不再可达**，
GitHub 侧的对象在垃圾回收前**仍可能按 SHA 被直接访问**；
要彻底清除需要删除并重建仓库、或联系 GitHub Support 清理。
本次涉及的是一台本机的弱口令（**该口令已不再使用**：应用已改为只授权单库的专用账号）
与文档路径里的用户名，据此判断风险有限；若要求彻底清除，可另做仓库重建。
