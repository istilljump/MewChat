...
| P2 | **`docs/demo-runbook.md` 明文写着本机开发库口令**（6 处），而仓库是公开的 —— 上传等于发布一个凭证 | 改为统一从 `MYSQL_PASSWORD` 环境变量读取，口令只落在被忽略的 `local.env.bat` 里。⚠️ **该口令已随早前提交进入公开仓库历史**，清理工作树不能撤回历史 —— 因此**做了历史重写**（见下方"隐私清理"）。若仍在别处复用那个弱口令，应视为已公开并轮换 |
...
1. **这台机器上 root 是被共用的**：本机另有约 8 个项目、36 处配置也用 `root`/同一口令连库（此处不逐一列名）。
...
仓库是公开的，核查后确认**几样真正要紧的东西从未进过仓库**：`local.env.bat`
（库口令与令牌密钥）从未被提交、令牌密钥与真实 API Key 在历史里查无、无截图/二进制文档、
无邮箱手机号。需要清理的是两类：

1. **文档里写过的本机库口令明文**：工作树在阶段 16 已改为读环境变量，
   但**历史提交里仍在**（`8feb56b` 起，即"删除"只能作用于之后的内容，删不掉过去）。
2. **本机用户名（真实姓名拼音）与其它作业项目的名字**：出现在 `AGENTS.md` 的
   m2 路径、`docs/demo-runbook.md` 的临时目录，以及我写进阶段 16 的
   "本机 root 被哪些项目共用"那段 —— 后者等于把机器上的目录结构一并公开。

处置：用 `git filter-branch --tree-filter` 对**全部 15 个提交**做字节级替换
（替换脚本放在仓库外，因为脚本自身含这些敏感串；按字节替换，UTF-8 的 .md 与 GBK 的 .bat 都覆盖）。
全程只改到 `AGENTS.md` 与 `docs/demo-runbook.md` 两个文件，**没有任何代码文件被动过**；
改完逐提交扫描（15 个提交的完整文件树）确认无残留，`./mvnw clean test` 仍是
`Tests run: 260, Failures: 0, Errors: 0, Skipped: 41`，然后强推。

**踩到并记下的两个坑**：

- **`--tree-filter` 的命令是在 filter-branch 的主壳里 `eval` 的，
  所以命令里写 `exit 0` 会让它处理完第一个提交就整体退出** ——
  退出码还是 0、日志看着像"跑过了"，但 `main` 的 SHA 根本没变。
  第一次重写就是这样"成功"的假象。**判断重写是否真的发生，不能看退出码，
  要看分支 SHA 是否变化，并对每个提交做一次扫描**（这次就是这么发现的）。
- 第一次把树过滤器的 stderr 丢进了 `/dev/null`，把问题一起丢掉了。
  自检类脚本要让错误显式暴露，宁可吵也不要静默。

**已知局限（已实测，必须知悉）**：强推**只是让旧提交不再可达，并没有把内容从 GitHub 抹掉** ——
改写并强推之后实测：`git fetch origin <改写前的SHA>` 仍能成功取回旧提交，
即任何持有旧 SHA 的人（改写前的克隆、之前拿到的 SHA）依然能读到当时的文档。
要让内容真正从 GitHub 消失，只有两条路：**删除并重建仓库**，或**联系 GitHub Support 清理**。
本次涉及的内容是一台本机的弱口令（**该口令已停用**：应用已改为只授权单库的专用账号）
与文档路径里的用户名，据此判断风险有限；若要彻底清除，建议重建仓库。

### 阶段 17 对话前端与启动日志噪声修复（2026-09-25）

两件事凑在一起：老板贴来一份一键启动的日志，问"为什么是这样"；
并要求照参考图做一个前端页面。
...
| 现象 | 根因 | 修法 |
| --- | --- | --- |
| 日志被 `==> Preparing: SELECT ...`、`Creating a new SqlSession` 刷屏，一次会话收尾上百行，真正的业务日志（状态机装配、工具注册、降级告警）被淹掉 | `mybatis-plus.configuration.log-impl: StdOutImpl`：**每一条 SQL 与每个 SqlSession 的生命周期都直接打到控制台**，且不受 `logging.level` 控制 | 改成 `Slf4jImpl`，并把 `com.mewchat.dao` 压到 `info`：平时不打 SQL，排查时调成 `debug` 即可 —— 不必再改配置文件 |
| 提示词与模型响应的完整内容进日志（含用户问题与回答原文） | `mewchat.llm.chat.log-requests/log-responses: true`（"开发期打开便于排查"） | 默认改 `false`。**理由是隐私而非噪声**：日志会被采集、转发、长期保存，等于把用户对话抄一份存进日志系统（与可观测那边"不默认打印 payload"是同一条原则）。需要时临时打开 |
| 同一条"API Key 仍是占位值"的告警连打三遍（另有一次 embedding 回退的，共三条） | 同一份 chat 密钥被三个模型 Bean（同步/流式/embedding 回退）各校验一次 | 按配置项名去重（`Set<String> warnedPlaceholderKeys`）：同一项只告警一次，不同项各自告警 |

顺带把 `embedding` 默认值的不匹配写成醒目注释：默认 `base-url` 复用 chat 的
`api.deepseek.com`，而 **DeepSeek 不提供 embeddings 接口**，默认模型名（OpenAI 的
`text-embedding-3-small`）与维度（1024，而它原生是 1536）都对不上 ——
**默认组合下向量化不可用是预期行为**，Milvus 默认也关着，检索走关键词通道。
这是"配置默认值之间的隐含矛盾"，之前只能靠读两份配置才看得出来。

> 复核：改完后启动日志里 `StdOutImpl` / `Creating a new SqlSession` / `Preparing:` 三类行数
> 全部为 **0**（实测计数）。

#### 二、对话前端（`src/main/resources/static/`）

由应用本体托管，无构建、无框架、无独立前端服务：`index.html` + `assets/app.css` + `assets/app.js`。
打开 `http://127.0.0.1:8080/` 即是登录页，登录后进入对话界面。

样式按参考图做：米黄纸底、3px 纯黑描边、硬阴影、橙色顶栏、珊瑚红用户气泡、
助手回答带边框卡片、引用渲染成 `[1]` 角标并列出《文档》第 N 段与相关度。

**新增的两个后端支撑**（前端要用，缺了做不出来）：

1. `GET /api/chat/sessions` —— "我的会话"列表（侧边栏"会话记录"）。
   <b>归属只认令牌</b>：请求里没有"查谁的会话"这类参数。这是列表接口最直接的攻击面 ——
   一旦接受 `userId`，一个拼错的参数就能把别人的会话列表列出来。
   `SessionView` 只暴露 sessionId / title / messageCount / lastMessageTime。
2. **首条用户消息兼作会话标题**。`conversation.title` 的列注释本来就写着"取首条提问"，
   但一直没实现（列表里只能显示会话ID）。写入用条件更新（`WHERE title 为空`），
   保证**只写一次** —— 否则连发两条消息时标题会被后一条覆盖，列表里那段对话的名字
   会随最后一句话变来变去。空会话（建了没说一句话）不进列表，否则每次点"新对话"
   都在侧边栏留一条空记录。

**前端的三个契约（照服务端已有约定写，各有一条注释说明为什么）**：

- **SSE 用 `fetch` + `ReadableStream` 自行解析，不用 `EventSource`**：后者无法自定义请求头，
  而所有接口都要 `Authorization: Bearer`。把令牌塞查询参数也能跑，但令牌会进访问日志。
- **`done` 事件是权威结果，整体替换而不是追加**：推送中途失败时已到客户端的片段收不回来，
  服务端会降级为兜底话术；若前端把片段当最终答案，用户看到的就和落库内容不一致。
- **进度提示用 `state` 事件**（"正在检索知识库…"这类中文状态名）——
  检索与工具调用都有真实耗时，期间一个字都没有会让用户以为卡住了。

**静态资源放行 + 接口仍然鉴权**：`SecurityConfig` 的免认证清单加了 `/`、`/index.html`、
`/assets/**`。浏览器第一次打开页面时还没有令牌，若页面本身也要认证，用户看到的是
一串 401 JSON 而不是登录页 —— 那就永远登录不了。放行的只是不含数据的骨架，
页面里每个数据请求仍然带令牌、仍然逐个校验归属。两条断言配对钉住这件事：
"无令牌能拿到页面"与"无令牌访问接口仍是 401"。

**点赞/点踩如实标注**：按钮渲染出来，点击提示"反馈功能尚未接入后端"。
不假装提交成功 —— 静默假成功会让用户以为反馈被记录了。

#### 三、验证

| 项 | 结果 |
| --- | --- |
| `./mvnw clean package`（默认） | **`Tests run: 267, Failures: 0, Errors: 0, Skipped: 44`** |
| `-Dmewchat.it.mysql=true`（门控） | **`Tests run: 267, Failures: 0, Errors: 0, Skipped: 0`** |
| 新增测试 | `ChatControllerTest` +4（会话列表归属与参数、无令牌 401、静态页免认证可访问、放行未波及数据接口）；`MysqlPersistenceFixesIntegrationTest` +3（标题只写一次、超长标题截断、我的会话=只看自己+排除空会话+按活跃排序） |
| 真容器 | 静态页 `/`、`/assets/app.js`、`/assets/app.css` 均 **200**；`/api/chat/sessions` 无令牌 **401** |
| 浏览器实测（真实 Chromium + 真库 + 本地假模型） | 登录（显示昵称"演示客户"）→ 侧边栏加载 6 条会话 → 新对话 → 提问"七天无理由退货怎么操作" → 流式回答 + 引用角标 `[1]` + 来源《七天无理由退换货规则》第 1 段（相关度 0.76）+ 置信度 0.83 → 列表刷新且标题自动变成"七天无理由退货怎么操作"；再问"你们支持开发票吗" → 兜底话术 + **"已转人工"角标** |
...
联调复查时在真实浏览器里发现：**连续问两次，页面上只剩最后一轮问答** ——
第一轮的问答被吃掉了。

根因在 `send()` 里多调了一次 `clearMessages()`：

```js
clearMessages();        // ← 清空整个对话区
appendUserBubble(text);
```

`clearMessages()` 的用途是"切换会话 / 新建会话时清屏"，被误用到了"继续发言"的场景。
后果是**多轮对话根本无法进行**：用户每问一句，前面所有内容消失。修法是拆出
`hideEmptyState()`（只收起空状态提示，不动已有消息），`send()` 改用它；
`clearMessages()` 仍只留给切换会话与新对话。

**这个 bug 是浏览器实测抓到的，不是单测**：接口层的测试全部通过 ——
服务端的行为完全正确（消息都落库了、历史接口也能完整回读），错的只有前端的渲染。
而我在第一次浏览器探测时其实已经看到了症状（第二次提问后 `cardCount` 只有 1），
当时误判为"新会话里本来就只有一轮"，直到复查代码才确认是清屏导致。
记下来是因为这类"服务端对、只有前端错"的缺陷，正是"接口测试全绿"最容易漏掉的一类。

修复后回归（真实浏览器 + 真库）：打开一段历史会话（1 轮问答）→ 再发一条 →
页面变成 **2 轮（用户气泡 2 / 回答卡 2）**，第一轮的引用与置信度仍在。
顺带做的一处调整：**回答失败时不再渲染点赞/点踩**（对一条没答出来的回复收集满意度没有意义）。

#### 五、连通性复查抓到的后端 bug：参数错误被报成 500

同一轮复查里用 curl 打边界，发现两类"调用方的问题被报成服务端故障"：

| 请求 | 修复前 | 修复后 |
| --- | --- | --- |
| `GET /api/chat/sessions?limit=abc`（参数类型错） | **HTTP 500 + 10003"系统繁忙"** | HTTP 200 + `10001 参数 limit 的取值类型不正确` |
| `POST /api/chat/sessions`（对着只读接口用错方法） | **HTTP 500 + 10003** | **HTTP 405** + `10001 请求方法 POST 不被支持，该接口支持：[GET]` |

根因与阶段 11 修过的"请求体格式错报成 500"完全同构，只是入口从请求体换成了查询参数与方法：
`MethodArgumentTypeMismatchException` / `MissingServletRequestParameterException` /
`HttpRequestMethodNotSupportedException` 都没有被 `GlobalExceptionHandler` 覆盖，一律落进兜底处理器。
后果也一样 —— 调用方以为是服务挂了而去重试，服务端的错误率与告警被这些本可避免的 500 污染。

修法：补三个处理分支（类型错与缺参数合成一个，因为它们只是入口不同、语义相同）。
两点细节：**不回显参数原值**（原值完全由调用方控制，回显进响应与日志没有必要，
还给了"把任意文本塞进日志"的机会，只说明是哪个参数、期望什么即可）；
"方法不支持"返回真正的 **405**，与 Security 的 401/403 保持同一种处理方式
（真正属于 HTTP 语义的错误就用真实状态码，业务失败才是 200 + 业务码）。

新增 2 项测试钉住（`ChatControllerTest`）：参数类型错 → 200 + 10001 且消息里点出参数名；
方法用错 → 405 + 10001。真容器复验同上表。

#### 六、假端点答非所问：判据用了共享数值（已修）

浏览器里问"MC202409240001 这单到哪了"，答的却是**商品**信息
（"无线蓝牙耳机 Pro 售价 499 元，库存 128 件"）。查日志确认**项目侧完全正确**：

```
意图识别结果：intent=ORDER_QUERY confidence=0.94 params=[orderNo]
路由决策：intent=ORDER_QUERY -> TOOL_CALL
工具调用完成：tool=order_query success=true
轨迹=[… ROUTE, TOOL_CALL, CONFIDENCE_CHECK, REPLY, END]
```

问题在**假端点的回复分支**（`FakeOpenAi.fragmentsFor`）：它按价格挑罐头回答
（`contains("499.00")` → 商品话术），而**订单工具的出参里也含这个金额**
（"实付金额：￥499.00"），于是订单问题命中了商品分支 ——
订单数据里根本没有"库存"这个字段，**内容错误但格式完全正常**，状态码与日志都看不出问题。

这与阶段 13 记过的"关键词不能拿整段提示词匹配"是同一类坑，只是这次共享的是**数值**而不是词。
修法：判据改成**各工具出参独有的字段名**（`库存：` 只有商品有、`最新轨迹：`/`运单号 `
只有物流有、`退换货政策：` 只有退款有、`订单状态：`/`实付金额：` 属订单），
商品再用价格区分；订单分支**从提示词里取出事实再复述**（订单号/状态/承运商），不编造。
...
| 位置 | 内容 |
| --- | --- |
| `service/ConversationCleanupService` | **新增**：删除会话时连同它的消息一起处理（跨聚合用例，见下） |
| `service/MessageService` | `deleteBySessionIds`（按会话物理删消息）、`firstUserMessages`（首个提问兜底） |
| `dao/.../MessageMapper` + `MessageMapper.xml` | 一条手写 SQL：按会话分组取最早那条用户消息 |
| `api/chat/ChatController` | `DELETE /api/chat/session/{id}`、`DELETE /api/chat/sessions`；列表标题兜底 |

**为什么删除编排要单独一个类**：删除要同时动会话与消息两张表，放进
`ConversationService` 就得让它依赖 `MessageService`，而 `MessageService` 已经依赖
`ConversationService`（反馈要校验会话归属）——**互相注入即循环依赖，Spring 启动直接失败**。
与其为了绕开它去直接注入对方的 Mapper（把分层当摆设），不如让这个协作型用例单独成类：
它只依赖两个服务、不被任何服务依赖，天然无环。两个方法都带
`@Transactional`：中途失败若只删一半，会出现"会话还在、历史空了"或"会话没了、消息还留在统计里"。

**删除语义（如实的取舍）**：会话走逻辑删除（全库统一），消息走**物理删除**
（`message` 表没有逻辑删除列）。结果是**用户删了就是真删**，客服后台也查不到这段对话。
真实客服系统通常需要保留会话用于审计与合规，那需要一个保留策略（例如只逻辑删除、后台仍可见）；
本项目按"用户可清理自己的数据"这一直觉实现，这一取舍写在类的注释里。

**清空全部**先查会话ID再按 ID 批量删消息（不按 userId 关联删消息：`message` 表里没有 userId，
归属只存在于会话上）；条数上限沿用会话列表口径（100），超出部分不会被删，
下次进入仍能看到 —— 比"悄悄只删一部分"更诚实。

#### 二、人性化改造（前端）

| 改进 | 为什么这么做 |
| --- | --- |
| 会话项悬停出现 `×`，**两步确认**（点一次变"确认删除"、3 秒不点自动复原） | 用 `window.confirm` 会打断操作流且样式无法统一；两步确认既防手滑，又不弹模态框 |
| 侧边栏"清空"，同一套两步确认 | 破坏性操作给一次"刹车"，但不给两次（点两次就够，不要输入框确认） |
| 空状态的示例问题**可点击直接发送** | 第一次用的人不用想"我该问什么"，也顺手示范了这台客服能答哪几类问题 |
| 回答卡片加 **复制**（⧉） | 客服回答里常有订单号、金额、政策期限，让用户手抄是最不人性的做法 |
| 本轮失败时给 **重试**（↻） | 失败后要用户重新打一遍字，是最容易被骂的设计 —— 问题原文就在手上 |
| 时间显示人性化（刚刚 / 12 分钟前 / 今天 15:04 / 昨天 / 09-24 16:12） | 服务端的 `yyyy-MM-dd HH:mm:ss` 在侧边栏里又长又难扫读 |
| 无标题的历史会话用**首个提问**兜底 | 那些会话建在"首句即标题"上线之前；只显示"（无标题）"等于把用户自己说过的话藏起来。**只读不写库**，标题仍为空，不掩盖"历史数据没标题"这个事实 |
| 触屏设备常显删除入口（`@media (hover: none)`） | 触屏没有 hover，靠悬停出现等于没有入口 |

#### 三、顺带修掉的两处"看不懂的报错"

1. **超长消息只报"HTTP 400"**：`/api/chat/send` 原先声明了 `produces = text/event-stream`，
   参数校验失败时错误体是 JSON、而客户端只声明接受事件流 → **内容协商失败**，
   Spring 只能回一个**空的 400**，用户完全不知道"消息太长"。
   修法：前端 `Accept` 改成 `text/event-stream, application/json`（成功收事件流、失败收 JSON），
   控制器文档写明这条契约，并加了一条测试钉住。
   > 附带发现：去掉 `produces` 声明并不能解决（协商看的是请求的 Accept），
   > 真正的原因在**客户端声明**这一侧 —— 记下来免得下次又去改服务端。
2. 前端原先对非 SSE 响应一律报"本轮没有收到内容"或"HTTP 400"，
   现在**优先采用响应体里的 message**，把服务端给出的原因原样呈现。

#### 四、验证

| 项 | 结果 |
| --- | --- |
| `./mvnw clean package`（默认） | **`Tests run: 286, Failures: 0, Errors: 0, Skipped: 51`** |
| `-Dmewchat.it.mysql=true`（门控） | **`Tests run: 286, Failures: 0, Errors: 0, Skipped: 0`** |
| 新增测试 | `ChatControllerTest` +5（删除走清理服务且取令牌用户、未登录拒绝且不进业务层、清空返回条数、无标题兜底、**流式接口校验失败要回可读原因**）；`MysqlPersistenceFixesIntegrationTest` +4（删除连带消息且之后读不到、删别人的被拒且数据不动、清空只动自己的、首个提问兜底查询取最早那条） |
| 真容器 | 无标题的 14 段历史会话标题全部兜底成功（未命名的 0 个）；删除返回"含 2 条消息"；删后历史接口 20002；重复删除同口径 |
| 浏览器实操 | 两步确认删除（第一次点只进入确认态、列表不变；第二次点才删，提示"已删除该会话（含 2 条消息）"）；清空（列表归零 + 空状态 + 4 个可点示例）；点示例问题直接发送并得到回答；失败卡片显示真实原因"message: 消息长度不能超过 2000 个字符"并带 ↻ 重试，点击后确实重发 |
| 未能验证（如实记录） | **复制按钮**：自动化是合成点击（非真实用户手势），浏览器拒绝剪贴板写入，因此只验证到"失败时如实提示"这条路径；真实用户点击下的复制需要人工确认 |

### 阶段 20 人性化第二轮：Markdown 渲染、恢复现场与前端第一份自动化测试（2026-09-26）

继续"功能尽量人性化"。这一轮的主项与副产物：

| 项 | 说明 |
| --- | --- |
| **回答按 Markdown 渲染** | 自实现子集（加粗 / 有序+无序列表 / 行内代码 / 分段），约 60 行，不引解析库。**安全模型**：正文先全量 HTML 转义，之后只做"文本 → 白名单标签"替换 —— 模型输出与知识库片段都按不可信内容处理，原文里的任何标签只会以文字出现。流式期间维护"累积原文"整段重渲，未闭合标记按字面显示、闭合后自然恢复 |
| **前端第一份自动化测试** | `tools/frontend-tests/render.test.js`（node 直跑，无框架）：从 app.js 按锚点切出 `escapeHtml`/`renderMarkdown` 原样执行，13 项断言钉住"任何 HTML 不能以标签形式存活"与"白名单格式确实被渲染"。**它当场抓到一个真缺陷**：中文顿号序号"2、第二条"（顿号后不空格）没被识别成列表 —— 正则随即修正（ASCII `1.` 必须后随空格，防止误吃"1.5万元"；`、` 不要求） |
| **刷新后自动打开最近会话** | "接着上次聊"是默认期望，让用户自己再点一下才能看到上文，等于把恢复现场的成本转嫁给用户。无历史会话时保持空状态（示例问题正好派上用场） |
| **字数计数器** | 服务端有 2000 字上限、超限整条拒绝 —— 与其发出去才报错，不如接近上限（1800）就看见数字、超限时变红并禁发。平时不显示（多数消息远够不着，常驻数字是噪声） |
| **智能滚动** | 流式输出的自动滚底只发生在用户本来贴着底部时（阈值 80px）；往上翻历史时不再被每秒几次的滚动拽回去 |
| **意图显示中文名** | 元信息里显示"知识问答/订单查询/…"而不是 KNOWLEDGE_QA；映射缺项时显示枚举原文而不是隐藏 |
| **移动端侧边栏抽屉** | 窄屏顶栏出 ☰，侧边栏改为滑出抽屉（此前窄屏直接 display:none，会话列表整个消失且无入口） |

**验证**：默认 **286 / 0 / 0（Skipped 51）**、门控 **286 / 0 / 0**；浏览器实操：赠品问题的回答以列表+加粗+行内代码渲染、刷新后自动接上文、计数器三态（隐藏/提醒/禁发）、窄屏抽屉可开合。

#### 这一轮揪出的两个环境级真问题

1. **假端点的流式响应是非法 JSON**（已修）。给赠品罐头回答加上多行 Markdown 后，
   流式立刻"已推送 0 字"—— `chunk()` 手拼 JSON 时**没有对内容转义**，
   内容里的裸换行直接生成非法 JSON，LangChain4j 解析失败。
   修法：`escape()` 统一处理反斜杠/引号/控制字符（`\n`→`\\n` 等），
   流式与非流式两条路径共用。教训与阶段 13"JSON 转义要还原成真字符"是同一族：
   **手拼 JSON 的地方，内容必须过转义函数**。
2. **MySQL 崩溃恢复后，全文索引对新插入行"静默失明"**（已修，见环境坑位）。
   昨天的 mysqld 随会话一起被杀（非正常停机），今天的实例里：旧行可查、
   **新插入的行 MATCH 永远查不到** —— "入库后立即检索"这条产品行为实际是坏的，
   且无任何报错。抓到它的是门控测试集（RAG 用例"入库即检索"全数失败），
   修复为删掉重建 `ft_content` 索引。这条印证了之前反复记录的判断：
   **"失败时也不报错"的行为只能靠真库测试兜住**。
...
- **MySQL 非正常停机后，InnoDB 全文索引对新插入行静默失明**（2026-09-26 实测）：
  mysqld 被直接终止再重启后，`knowledge_chunk` 的旧行 MATCH 可查，
  **新 INSERT 的行 MATCH 永远返回空** —— "入库后立即检索"这条产品行为整个失效且无报错。
  `OPTIMIZE TABLE ... WITH innodb_optimize_fulltext_only` 重建**不够**，
  必须删掉重建索引：`ALTER TABLE knowledge_chunk DROP INDEX ft_content;
  ADD FULLTEXT INDEX ft_content (content) WITH PARSER ngram;`。
  重建前用"新插入行 + 中文唯一词"验证，重建后同一探针立即可查。
  顺带：用 curl 直接探 MySQL/HTTP 时中文会按 GBK 发出（见阶段 11），
  **探针请用 Python 以 UTF-8 发送** —— 这轮排查时探针本身被编码坑了半小时。
