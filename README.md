...
启动完成后打开 **http://127.0.0.1:8080/** 就是对话页面（见下文"打开浏览器就能用"）。
...
应用本体托管了一个对话页面，**不需要另起前端服务、也没有构建步骤**：
...
与一个复制按钮；会话可以单条删除（悬停出现 ×，两步确认）或一键清空（"清空"同样两步确认），
空状态里的示例问题点一下就能直接问。源码在 `src/main/resources/static/`。
...
# 我的会话列表（对话页侧边栏用的同一个接口）
curl -s http://127.0.0.1:8080/api/chat/sessions -H "Authorization: Bearer $TOKEN"
