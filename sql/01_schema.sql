...
-- 目标库名由调用方指定（脚本内不写 USE），统一用：
--   mysql -u <用户> -p --default-character-set=utf8mb4 --database=mewchat < 本脚本
-- 刻意不写 USE 语句：写死库名时，若调用方指定了别的库，建表会静默落到写死的那个库上，
-- 六个脚本各写各的、报错也没有 —— 而"未选择数据库"是一个立刻能看见的错误。
--
-- 全局约定：
